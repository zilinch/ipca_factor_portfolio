...
def ipca_step_t(t, window_size, df_ipca, unique_dates, K, characteristics, log_fp, res_fp):
    ...
    date_to_predict = unique_dates[t].strftime('%Y-%m-%d')
    ...
    print (np.sum(w_individual[w_individual>0]))
    
    ret_t = (V_t @ r_t).T @ OptimalPortfolioWeights_t
    excess_ret_t = (V_t @ excess_r_t).T @ OptimalPortfolioWeights_t
    sigma_r_t = np.std(r_t)
    # print (f'======Max_return: {t}: {ret_t}======')                  
    
    df_results = pd.DataFrame([[date_to_predict, ret_t, excess_ret_t, sigma_r_t]],
                          columns=['Date', 'P_Return', 'P_Excess_Return', 'Sigma_Return'])
    with lock:
...
def main():
    ...
    # preprocess
    fn = "/gpfs/home/zilinchen/ipca_factor_portfolio/data/kelly_data_without_nanocap.p"
    ...
    # args_list = [(501, window_size, df_ipca, unique_dates, K, characteristics, log_fp, res_fp)]
