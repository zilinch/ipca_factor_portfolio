...
from ipca_utils import impute_w_median, IPCA_factor_v2
...
def ipca_step_t(t, window_size, df_ipca, unique_dates, characteristics, log_fp, ipca_fp):
    '''
    ...
    # Fit ipca
    try:
        Gamma, Factors, r_t, excess_r_t, X_last = IPCA_factor_v2(window_data, characteristics)
...
def main():
    ...
    window_size = 120 # in months
    unique_dates = sorted(df_ipca['eom'].unique()) # unique dates
    T = len(unique_dates)
   
    current_date = datetime.now().strftime('%Y-%m-%d')
    log_fp = "logs/"+f"{current_date}-w{window_size}-log-error.txt"
    ipca_fp = f"results/IPCA_intermediates_v2/"
    
    if not os.path.exists(ipca_fp):
        os.makedirs(ipca_fp)
    
    args_list = [(t, window_size, df_ipca, unique_dates, characteristics, log_fp, ipca_fp) \
                        for t in range(window_size, T)]
    
    lock = Lock()
    # 5-6 processes looks good for 32GB of RAM
    with Pool(processes=5, initializer=init_pool_lock, initargs=(lock,)) as pool:
