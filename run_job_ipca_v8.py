...
from ipca_utils import IPCA_factor_v8
...
def ipca_step_t(t, window_size, df_ipca, unique_dates, K, characteristics, log_fp, ipca_fp):
    ...
    # Fit ipca
    try:
        Gamma, Factors, r_t, excess_r_t, X_last = IPCA_factor_v8(window_data, characteristics, K)
...
def main():
    ...
    window_size = 240 # in months, only defines starting date because this is expanding window
    ...
    ipca_fp = f"results/IPCA_intermediates_v8/"
    ...
    with Pool(processes=2, initializer=init_pool_lock, initargs=(lock,)) as pool:
